...
class MQTTAPI {
     ...
     */
    constructor(broker_url, clientCollection, topicCollection, handlers) {
        this.clientCollection = clientCollection;
        this.topicCollection = topicCollection;
        this.handlers = handlers;
        this.workers = {};
        this.metadata = new mqtt_metadata_1.MQTTMetaData();
    ...
    createHandlerMap(handlers) {
        ...
        const topicMap = {
            ...
            '+/prog': (username, payload, worker) => undefined // special case: handled in topicDispatch: TODO
        };
    ...
    getWorker(username) {
        if (!this.workers[username]) {
            this.workers[username] = new mqtt_protocol_1.MQTTWorker(username, this.mqtt_client, MAX_PACKET_SIZE, this.metadata);
        }
