...
class MQTTAPI {
     ...
     */
    constructor(broker_url, clientCollection, topicCollection, handlers, maxPacketSize, username, password) {
        ...
        this.hi_protocol_handler = (username, topic, payload, worker) => {
        ...
        const options = {
            ...
            password: password || utils_1.utils.randomSecret(30),
        };
    ...
    createHandlerMap(handlers) {
        ...
        const topicMap = {
            ...
            'fixeddatatest': (username, topic, payload, worker) => worker.topic_fixeddatatest(payload),
            'acktest': (username, topic, payload, worker) => worker.topic_acktest(payload),
            'acktest/ack': (username, topic, payload, worker) => worker.topic_acktest_ack(payload),
            'selftest': (username, topic, payload, worker) => worker.topic_selftest(payload),
            '+/ack': (username, topic, payload, worker) => undefined,
            '+/prog': (username, topic, payload, worker) => undefined // special case: handled in topicDispatch
        };
        // Additionaly arbitrary user defined topics and handlers.
        handlers.topic_list.forEach(topic_desc => {
            const handler = handlers['topic_' + topic_desc.topicName.replace('/', '_').replace('#', 'x')];
            let wrapped_handler;
            if (!handler) {
        ...
        Expected: 'topic_ +  ${topic_desc.topicName}(username: string, payload?: Buffer)'`);
            ...
            if (topic_desc.type && topic_desc.type === 'fixeddata') {
                wrapped_handler = (username, topic, payload, worker) => {
                ...
                wrapped_handler = (username, topic, payload, worker) => handler(username, topic, payload);
            }
    ...
    topicDispatch(topic, message) {
        ...
        if (!handler) {
            ...
            if (ackM) {
                ...
                handler = (username, topic, payload, worker) => {
