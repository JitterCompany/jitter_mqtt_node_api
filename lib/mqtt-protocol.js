...
const mqtt_protocol_test_1 = require("./mqtt-protocol-test");
 ...
 */
class MQTTWorker {
    constructor(username, mqtt_client, max_packet_size, metadataStore) {
    ...
    createSendTransfer(topic, data) {
        const packets = utils_1.utils.createFixedDataPacket(Buffer.from(data), this.max_packet_size);
        this.sendPackets(topic, packets);
    }
    sendPackets(topic, packets) {
        ...
        state.data.forEach(packet => this.mqtt_client.publish(topic, packet));
        return true;
    }
    /**
    ...
    topic_acktest(payload) {
        ...
        if (ack) {
            ...
            this.mqtt_client.publish(utils_1.utils.getAckTopicName(topic), ack);
            if (ack.readUInt16LE(0) === 0xFFFF) {
                ...
                const data = utils_1.utils.createFixedDataPacket(Buffer.from(test_str.repeat(6)), 8);
                console.log('send acktest data to ', sendtopic);
                this.sendPackets(sendtopic, data);
            }
        }
    }
    addTask(topicHandler, topic, payload_in) {
    ...
    fixedDataProgessHandler(topic, payload) {
    ...
    fixedDataAckHandler(topic, payload) {
        ...
        if (state) {
            if (ack === state.total_packets) {
                ...
                if (state.retry()) {
                    state.data.slice(ack).forEach(packet => this.mqtt_client.publish(topic, packet));
                }
                else {
    ...
    fixedDataReceiveHandler(topic, payload) {
        ...
        if (packet_number === 0) {
        ...
        else if (state.total_packets && (state.total_packets !== total_packets)) {
            ...
            this.mqtt_client.publish(utils_1.utils.getAckTopicName(topic), rmsg);
            return;
        }
        if (valid) {
            // add payload after header to data store
            state.data.push(payload.slice(4));
            this.metadataStore.updateProgress(this.username, topic, packet_number + 1, total_packets);
        }
        if (packet_number === (state.total_packets - 1)) {
            ...
            if (state.tranfser_complete()) {
                ...
                if (utils_1.utils.checkCRC32(all_data)) {
                    const data = all_data.slice(0, -4);
                    console.log('transfer complete');
                    complete = true;
                    state.reset_retries();
                    rmsg.writeUInt16LE(state.packets_received(), 0);
                    this.metadataStore.finishProgress(this.username, topic);
                }
                else {
...
//# sourceMappingURL=mqtt-protocol.js.map