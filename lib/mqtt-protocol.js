...
class MQTTWorker {
    ...
    createSendTransfer(topic, data) {
        const packets = utils_1.utils.createFixedDataPacket(new Buffer(data), this.max_packet_size);
    ...
    topic_acktest(payload) {
        ...
        if (ack) {
            ...
            if (ack.readUInt16LE(0) === 0xFFFF) {
                ...
                const data = utils_1.utils.createFixedDataPacket(new Buffer(test_str.repeat(6)), 8);
                console.log('send acktest data to ', sendtopic);
                this.sendPackets(sendtopic, data);
            }
        }
    }
    addTask(topicHandler, payload_in) {
