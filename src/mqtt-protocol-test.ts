...
function test0_normal() {
  ...
  const data = utils.createFixedDataPacket(Buffer.from(test_str.repeat(4)), 8);
  return {
    title: 'Test 0 normal',
    stages: [
    {
...
function test1a_skip() {
  ...
  const test_str = 'TestData';
  const data1 = utils.createFixedDataPacket(Buffer.from(test_str.repeat(4)), 8);
  const data2 = utils.createFixedDataPacket(Buffer.from(test_str.repeat(6)), 8);
  data1.splice(2, 1);
  return {
...
function test2_early_restart() {
  ...
  const test_str = 'TestData';
  const data1 = utils.createFixedDataPacket(Buffer.from(test_str.repeat(4)), 8);
  const data2 = utils.createFixedDataPacket(Buffer.from(test_str.repeat(3)), 8);
...
function test3_change_length() {
  ...
  const test_str = 'TestData';
  const data_short = utils.createFixedDataPacket(Buffer.from(test_str.repeat(4)), 8);
  const data_long = utils.createFixedDataPacket(Buffer.from(test_str.repeat(6)), 8);
...
function test4_duplicate() {
  ...
  const test_str = 'TestData';
  let data_dup = utils.createFixedDataPacket(Buffer.from(test_str.repeat(5)), 8);
...
function test5_crc() {
  ...
  const test_str = 'TestData';
  const data_a = utils.createFixedDataPacket(Buffer.from(test_str.repeat(5)), 8);
  const test_str2 = 'blabla12';
  const data_b = utils.createFixedDataPacket(Buffer.from(test_str2.repeat(5)), 8);
  data_a[2] = data_b[2];
  return {
...
function test6_crc() {
  const test_str = 'TestData';
  const bad_packet = utils.createFixedDataPacket(Buffer.from(test_str.repeat(2)), 8);
  bad_packet[1].writeInt32LE(0xDEAD, 4);
  bad_packet[2].writeInt32LE(0xC0DE, 4);
  const good_packet = utils.createFixedDataPacket(Buffer.from(test_str.repeat(2)), 8);
  return {
